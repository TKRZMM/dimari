 ...
 * Time: 15:32
 */
class DimariExp_TKRZ_FTTH extends Dimari_TKRZ_FTTH
{
...
//		// IDEBUG pre - tag
//		echo "<pre><hr>";
//		print_r($this->globalData['CUSTOMER_ID_Array']);
//		echo "<hr></pre><br>";
		...
		foreach($this->globalData['CUSTOMER_ID_Array'] as $curCustomerID => $customerArray) {
			...
			foreach($customerArray['CONTRACT_ID'] as $curContractID => $contractArray) {
				...
				// Durchlauf Products
				$boolGotWeb = false;	// 1. Frage ... Hat der Kunde fiberWeb?
				$boolGotFon = false; 		// 2. Frage ... Hat der Kunde Fon?
				$boolGotTV = false;			// 3. Frage ... Hat der Kunde TV?
				$boolGotDocsis = false;		// 4. Frage ... Docsis?
				$boolGotGenexis = false;	// 5. Frage ... Genexis?
				$boolGotSpeedUp = false;	// 6. Frage ... Hat Speedupgrade?
				...
				if ((isset($contractArray['PRODUCT_ID'])) && (count($contractArray['PRODUCT_ID']) > 0)) {
					...
					// Art feststellen ... Fon / Web / TV ... anhand der ACCOUNTNO
					// 85200 Fon
					// 85300 Web
					// 85400 TV
					// 85500 Mietgerät
					foreach($contractArray['PRODUCT_ID'] as $curProductID => $productArray) {
						...
						// 1. Frage ... Hat der Kunde fiberWeb?
						if ($productArray['ACCOUNTNO'] == '85300')
							$boolGotWeb = true;


						// 2. Frage ... Hat der Kunde Fon?
						if ($productArray['ACCOUNTNO'] == '85200')
							$boolGotFon = true;


						// 3. Frage ... Hat der Kunde TV?
						if ($productArray['ACCOUNTNO'] == '85400')
							$boolGotTV = true;


						// 4. Frage ... Docsis?
						if (in_array($curProductID, $this->setDocsisByProductID[$this->setExportType]))
							$boolGotDocsis = true;

						// 5. Frage ... Genexis?
						if (in_array($curProductID, $this->setGenexisByProductID[$this->setExportType]))
							$boolGotGenexis = true;

						// 6. Frage ... Hat Speedupgrade?
						if ( ($curProductID == '10028') || ($curProductID == '10072') )
							$boolGotSpeedUp = true;
						...
						if ((isset($productArray['COV_ID'])) && (count($productArray['COV_ID']) > 0)) {
							foreach($productArray['COV_ID'] as $curCOV_ID => $covIDArray) {
								...
								$curDataSet['CUSTOMER_ID'][$curCustomerID]['VOIP_EXT_PRODUKT_ID'] = '';
					...
					}	// END // Art feststellen ... Fon / Web / TV ... anhand der ACCOUNTNO
					...
					if ($boolGotDocsis){
						if ($boolGotFon) {
							$curProduktBezeichnung = 'DOCSIS FON';
							$curProduktExtID = '833';
							if ($boolGotSpeedUp)
								$curOption_1 = '59';
						}
						...
						if ($boolGotWeb){
							$curProduktBezeichnung = 'DOCSIS WEB';
							$curProduktExtID = '834';
							if ($boolGotSpeedUp)
								$curOption_1 = '60';
						}

						if ( ($boolGotWeb) && ($boolGotFon) ){
							$curProduktBezeichnung = 'DOCSIS FON/WEB';
							$curProduktExtID = '835';
							if ($boolGotSpeedUp)
								$curOption_1 = '75';
						}
					...
					// Genexis Kunde
					elseif ($boolGotGenexis){
						if ($boolGotFon) {
							$curProduktBezeichnung = 'FIBER4business VoIP';
							$curProduktExtID = '810';
						}

						if ($boolGotWeb){
							$curProduktBezeichnung = 'FIBER2home Internet Vorab';
							$curProduktExtID = '832';
						}

						if ( ($boolGotWeb) && ($boolGotFon) ){
							$curProduktBezeichnung = 'FIBER2home BASIC';
							$curProduktExtID = '830';
							if ($boolGotSpeedUp)
								$curOption_1 = '56';
						}

						if ( ($boolGotWeb) && ($boolGotFon) && ($boolGotTV)){
							$curProduktBezeichnung = 'FIBER2home PREMIUM';
							$curProduktExtID = '831';
							if ($boolGotSpeedUp)
								$curOption_1 = '58';
						}
						...
						// echo "Typ unbekannt (Docsis oder Genexis?";
						$curProduktBezeichnung = '';
						$curProduktExtID = '';
					}

					// echo "------------->$curProduktBezeichnung $curProduktExtID";


					// Setze Optionen:
					$curDataSet['CUSTOMER_ID'][$curCustomerID]['OPTION_1'] = $curOption_1;
					$curDataSet['CUSTOMER_ID'][$curCustomerID]['OPTION_2'] = $curOption_2;
					$curDataSet['CUSTOMER_ID'][$curCustomerID]['OPTION_3'] = $curOption_3;


					// Setze Dienst-Bezeichnung
					$curDataSet['CUSTOMER_ID'][$curCustomerID]['DIENST_BEZEICHNUNG'] = $curProduktBezeichnung;
					...
					// Setze Ext_Produkt_ID
					$curDataSet['CUSTOMER_ID'][$curCustomerID]['EXT_PRODUKT_ID'] = $curProduktExtID;

					// Setze ClientID ... laut R. Schwarz ist die leer (weil wir hier keine WiSoTEL - Kunden haben)
					$curDataSet['CUSTOMER_ID'][$curCustomerID]['CLIENT_ID'] = '0';
		...
		// IDEBUG pre - tag
//		echo "<pre><hr>";
//		print_r($curDataSet['CUSTOMER_ID']);
//		echo "<hr></pre><br>";

		return true;

	}	// END public function mainToExcel()
